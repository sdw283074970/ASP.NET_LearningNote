//Q: 什么是Validation？
//A: Validation即验证，验证的根本目的在于确保整个程序按照逻辑运行，不符合逻辑的执行命令(即没有被验证的执行命令)将会中断程序，抛出Invalidation的异常。
...
namespace Vidly.Models
{
  ...
  //2.添加验证逻辑。在ASP.NET中，有一个静态类布尔属性ModelState.IsValid来储存模型的验证状态，通过这个属性可以判断整个程序的运行是否符合逻辑。如果
    ...
    //我们可以通过ModelState.IsValid的值判断程序是否有问题。如果有问题，就需要返回带错误信息的视图给用户。
    
    //如项目需求为在添加顾客页面和编辑页面中，即使用户输入不符合逻辑的数据，程序也不能崩溃，还要能告诉用户出错的地方。
    
    //据分析，相关的视图为“CustomerForm”，相关的Controllor和Action为"Customer/Save"。我们用修改Save()方法即可判断程序的验证性。代码如下：
