...
每当用户在QB中新建一个公司的时候，Intuit就会为新建的这个公司指定一个`eal ID`作为唯一标识。这个ID可以在QBO中的**Billing & Subscription**页面和**Account and Setting**中找到。在运行时，可以通过在OAuth认证过程中的重定向连接URL中的`realmId`参数中找到。
...
如`2015-07-24T10:33:39-07:00`指太平洋时间(-7H)2015年7约24日陵城3点33分39秒
